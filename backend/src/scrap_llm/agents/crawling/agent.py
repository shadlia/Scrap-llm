...
async def create_agent():
    ...
    crawling_agent_with_tool = LlmAgent(
        model=MODEL_NAME,
        name="crawling_agent",
...
# --- 7. Run Interactions ---
async def run_crawling_agent(url: str) -> list[str]:
    ...
    crawling_agent_with_tool, exit_stack = await create_agent()
    crawling_runner = Runner(
        agent=crawling_agent_with_tool,
        app_name=APP_NAME,
        session_service=session_service,
    )

    stored_output = await call_agent_and_print(
        crawling_runner,
        crawling_agent_with_tool,
        SESSION_ID_TOOL_AGENT,
        f'{{"url": "{url}"}}',
        session_service,
    )
