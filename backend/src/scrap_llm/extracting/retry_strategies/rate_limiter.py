...
class RateLimiterRetry(BaseRetryStrategyImpl):
    async def retry_failed_urls(
        self, failed_urls: List[str], save_path: str
    ) -> tuple[list[str], list[Any]]:
        print("Retrying failed URLs with rate limiter:", len(failed_urls))
        still_failed_urls = []
        successful_products = []
        ...
        async with AsyncWebCrawler(
            ...
            async for result in await crawler.arun_many(
                ...
                if not result.success:
                    print(f"Failed to fetch URL with rate limiter: {result.url}")
                    still_failed_urls.append(result.url)
                    continue
                products = await self.process_product(result, save_path)
                if not products:
                    print(f"Failed to process URL with rate limiter: {result.url}")
                    still_failed_urls.append(result.url)
                else:
    ...
    async def retry(
        ...
        """
        Retry failed URLs using rate limiter strategy.
        """
